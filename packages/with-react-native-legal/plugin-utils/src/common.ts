    ...
    (acc, dependency) => {
      try {
        ...
        if (!localPackageJsonPath) {
          console.warn(`[with-react-native-legal] skipping ${dependency} could not find package.json`);
          return acc;
        }
        ...
        console.warn(`[with-react-native-legal] could not process package.json for ${dependency}`);
      }
