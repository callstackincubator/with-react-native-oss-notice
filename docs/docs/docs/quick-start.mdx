...
<PackageManagerTabs command="install with-react-native-legal" />
...
{
  "expo": {
++  "plugins": ["with-react-native-legal"]
  }
}
```
...
<PackageManagerTabs command="react-native with-react-native-legal" />
...
:::warning
`with-react-native-legal` only detects licenses for dependencies listed in the app's `package.json`.   It won't include dependencies used only in monorepo packages unless they are also in `package.json`. To include more licenses, add those dependencies to the app's `package.json`.  
:::